...
################################################################################
# plot simulation results for two examples with adjust = 1,3 observations per density
################################################################################
# read simulation results
sim_results <- readRDS("simulation_results/simulation_30_1001")
...
cov_data_list <- lapply(1:3, function(i){
...
mean_plot_data_list <- lapply(1:3, function(i){
...
density_plot_data_list <- lapply(1:3, function(j){
...
g3 <- ggplot(data = density_plot_data, aes(x = x, y = y, group = idx, color = idx)) + geom_path() +
  facet_grid(cols = vars(type)) + scale_color_discrete(rainbow(30)) + ylab("density") + 
  theme(legend.position = "none")
g3
################################################################################
pdf("../../Figures/simulation_example_bw_3.pdf", width = 6)
...
################################################################################
bw_vec <- c(10, 15, 20, 25, 30)
dist_data_list <- lapply(bw_vec, function(m){
  ...
  dist_data_list <- lapply(file_names, function(file){
    ...
    dist_data_list <- lapply(2:3, function(j){
...
dist_data<- do.call("rbind", dist_data_list)
dist_data$bw <- as.factor(dist_data$m/10)
dist_data$method <- factor(dist_data$method, levels = type_levels) 

g_mean <- ggplot(data = dist_data[dist_data$fun == "mean",], aes(x = bw, y = x, fill = method)) + geom_boxplot() +
  ylab("dist mean") + xlab("bandwidth adjustment factor") + ggtitle("Distance of mean estimate to oracle estimate") +
  theme(legend.position = "top") + scale_fill_brewer(palette="BuPu")

g_cov <- ggplot(data = dist_data[dist_data$fun == "cov",], aes(x = bw, y = x, fill = method)) + geom_boxplot() +
  ylab("dist covariance") + xlab("bandwidth adjustment factor") + ggtitle("Distance of covariance estimate to oracle estimate" ) +
  theme(legend.position = "top") + scale_fill_brewer(palette="BuPu")

pdf("../../Figures/simulation_dist_to_oracle_bw.pdf", width = 10, height = 4)
